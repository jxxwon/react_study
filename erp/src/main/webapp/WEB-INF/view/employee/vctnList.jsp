...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
		...
		<c:if test="${vctnCnt > 0 }">
			...
			<c:forEach items="${vctnList}" var="list">
				<tr>
					<td><a href = "javascript:vctnDetailModal('${list.seq}');">${nRow+1}</a></td>
					<td>${list.vctnName}</td>
					<td>${list.loginId}</td>
					<td>${list.name}</td>
		...
		<input type="hidden" id="totcnt" name="totcnt" value="${vctnCnt}">
